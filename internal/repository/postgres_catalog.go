...
func (r *postgresCatalogRepository) CreateCategory(c *domain.Category) error {
	query := `
		INSERT INTO categories (id, establishment_id, name, description, "order", created_at)
		VALUES ($1, $2, $3, $4, $5, $6)
	`
	_, err := r.db.Exec(query, c.ID, c.EstablishmentID, c.Name, c.Description, c.Order, c.CreatedAt)
	return err
}

func (r *postgresCatalogRepository) GetCategoriesByEstablishment(estID uuid.UUID) ([]*domain.Category, error) {
	query := `SELECT id, establishment_id, name, description, "order", created_at FROM categories WHERE establishment_id = $1 ORDER BY "order" ASC`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.EstablishmentID, &c.Name, &c.Description, &c.Order, &c.CreatedAt); err != nil {
			return nil, err
		}
...
func (r *postgresCatalogRepository) GetCategoryByID(id uuid.UUID) (*domain.Category, error) {
	query := `SELECT id, establishment_id, name, description, "order", created_at FROM categories WHERE id = $1`
	var c domain.Category
	err := r.db.QueryRow(query, id).Scan(&c.ID, &c.EstablishmentID, &c.Name, &c.Description, &c.Order, &c.CreatedAt)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (r *postgresCatalogRepository) UpdateCategory(c *domain.Category) error {
	query := `UPDATE categories SET name = $1, description = $2, "order" = $3 WHERE id = $4`
	res, err := r.db.Exec(query, c.Name, c.Description, c.Order, c.ID)
	if err != nil {
		return err
	}
